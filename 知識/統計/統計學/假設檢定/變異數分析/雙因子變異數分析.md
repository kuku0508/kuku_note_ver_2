...
交互作用&\sum^g_{k=1}\sum^b_{\ell=1}n_{\ell k}(\bar{x_{\ell k}}-\bar{\bar{x}})^2-&(g-1)\cdot(b-1)&\frac{SSAB}{(g-1)\cdot(b-1)}&\frac{MSAB}{}\\
&SSA-SSB\\

組內（error）&\sum^g_{\ell=1}\sum^b_{k=1}\sum^n_{r=1}(x_{\ell k r}-\bar{x_{\ell k}})&(n-1)\cdot g\cdot b&\frac{SSE}{(g-1)\cdot(b-1)}\\
\hline
