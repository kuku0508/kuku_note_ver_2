...
ANOVA&SS&DF&MS&F\\
\hline
因子一&\sum_{\ell=1}^g b(\bar{x_\ell.}-\bar{\bar{x}})^2&(g-1)&SSA\\
因子二&\sum^b_{k=1}g(\bar{x_{.k}}-\bar{\bar{x}})^2
&(b-1)\\
組內（error）&\sum^g_{k=1}\sum^b_{\ell=1}(x_{\ell k}-\bar{x_{\ell .}}-\bar{x_{.k}}+\bar{\bar{x}})^2&(g-1)\cdot(b-1)\\
\hline
