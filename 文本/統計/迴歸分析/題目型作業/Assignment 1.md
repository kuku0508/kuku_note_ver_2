...
$$
\begin{align}
Var(\hat{\beta}_1)&=Var(\beta_1+\frac{\sum(x_i-\bar{x})\epsilon_i}{\sum(x_i-\bar{x})^2})\\
&=Var(\frac{\sum(x_i-\bar{x})\epsilon_i}{\sum(x_i-\bar{x})^2})\\\\
&=\frac{1}{\sum(x_i-\bar{x})^2}Var(\sum(x_i-\bar{x})\epsilon_i)
\end{align}
$$
