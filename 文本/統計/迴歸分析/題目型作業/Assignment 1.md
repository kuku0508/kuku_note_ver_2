...
&=\frac{1}{\sum(x_i-\bar{x})^2}E\left[\cancel{\sum(x_i-\bar{x})\hat{\beta}_0}^{\,\,0}+\sum(x_i-\bar{x})\hat{\beta}_1x_i\right]\\
&=\frac{1}{\sum(x_i-\bar{x})^2}\left[\sum(x_i-\bar{x})\hat{\beta}_1x_i)\right]
\end{align}
$$
