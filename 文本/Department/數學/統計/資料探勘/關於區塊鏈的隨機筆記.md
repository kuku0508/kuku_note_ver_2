...
每一個區塊除了交易紀錄外，前面其實還有一個部分叫做區塊頭（Head），區塊頭包含的是區塊的標籤，裡面有例如「前一個區塊的哈希值（prev_block）」、「隨機數（nonce）」、「時間戳記（time）」、「Merkle根（maki_root）」。請將區塊頭進行兩次SHA256計算，求其值。誰先算出這個值，誰就有資格提出自己的區塊
