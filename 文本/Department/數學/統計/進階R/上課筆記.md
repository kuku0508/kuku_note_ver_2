...
雖然不太確定為什麼會需要乘以1.83，我自己猜想可能是要讓較大的分數的權重高一點，但還是不確定為什麼要有這個權重。自己猜想可能是透過歷史數據進行迴歸分析得到的最佳解。

而報告也點出了當今天出現了戰力保留/兩隊分差過大的情況，會讓勝敗場預測的結果失真，能感覺的出來兩位在這次棒球中的觀察入微。因此兩位也是用了類似截尾的方式來修改了得失分的計算。但跟前面的權重問題一樣，這邊選擇六場為閥值的原因，我有一點不太確定為什麼。也許在無法得到最佳閥值的情況下，可以多嘗試不同閥值，來看預測能力是否可以達到更好的效果。當然最好的情況還是可以透過統計方法來得到一個較佳的閥值，又或是使用別的統計方法。

最後看到報告後面的代入實際數據的修正，有發現在30筆的數據裡面，有12筆數據的勝率比起修正前來說誤差更大。在勝率預測跟原本的勝率預測比較，看起來並沒有達到比較有效的預測能力。也許透過更嚴謹的驗證，我們可以將這個新方法來得到更好的預測效果
12 30