...
$\therefore$ $T(\underline{X})$ is sufficient statistic.
...
Hence, $T(\underline{X})$ is a complete statistic.
...
&Q(\theta)=\ln(1-\theta)\\
&\because 0<\theta<1\\
&\therefore \lim_{\theta\rightarrow 0^+}\ln(\)=0\rightarrow\ln(1)=0\\
&\quad\theta=1\rightarrow\ln(0)=-\infty\\
&\Rightarrow-\infty<Q(\theta)<0
\end{align}
$$
Since the Geometric distribution belongs to a one-parameter exponential family, and the natural parameter space $Q(\theta)\in(-\infty,0)$
Hence, $T(\underline{X})$ is a complete statistic.
...
&\text{Since the family of distributions is an Exponential Family,}\\

&\text{and the natural parameter space } \eta \in (-\infty, 0) \text{ contains an open interval (non-degenerate),}\\

&\text{therefore, the statistic } T = \sum_{i=1}^n X_i \text{ is a complete sufficient statistic for } \theta.
