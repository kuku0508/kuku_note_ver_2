...
#### 問他的MLE是不是最小充分
$\because$ $\hat{\theta}=\frac{1}{\bar{X}+1}$ is a one-to-one function of $T(\underline{X})$ , and $T(\underline{X})$ is sufficicent 
$\therefore$ $\hat{\theta}$ is also sufficicent of $\theta$.
$\because$ $\hat{\theta}$ has unique solution
$\therefore$
$\because$ $\hat{\theta}$ is exists uniquely and suffcicent.
$\therefore$ $\hat{\theta}$ is minimal sufficient statistic.
