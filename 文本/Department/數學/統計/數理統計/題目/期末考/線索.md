...
&\therefore \text{Hence,the Geometric distribution is belong to Exponential family}
\end{align}
$$


#### 請用充分統計量的定義證明
我們需要一個統計量來帶我們接下來的題目，我們這邊先用總和$T=\sum
...
$$
\begin{align}
f_{\underline{X}\mid T}(\underline{x}\mid t)=\frac{f_\underline{X}(x;\theta)}{f_T(t;\theta)}&=\frac{\prod^n_{i=1}\theta(1-\theta)^x}{}
\end{align}
$$