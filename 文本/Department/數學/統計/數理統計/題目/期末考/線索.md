...
Since $X_i$ represents the number of failures before the i-th success, the sum T=
$$
\begin{align}
f_{\underline{X}\mid T}(\underline{x}\mid t)=\frac{f_\underline{X}(x;\theta)}{f_T(t;\theta)}&=\frac{\prod^n_{i=1}\theta(1-\theta)^x}{}(\because T\sim NegBin())
\end{align}
$$