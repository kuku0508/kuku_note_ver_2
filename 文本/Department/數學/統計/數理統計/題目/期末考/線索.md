...
Let g be a function such that $E[g(x)]=0$ 
$$
\begin{align}
E[g(x)]&=\sum^\infty_{t=0}g(t)\binom{n+t-1}{t}\cdot\theta^n(1-\theta)^t\\
&\because \theta^n>0\\
&=\sum^\infty_{t=0}{g(t)\binom{n+t-1}{t}}(1-\theta)^t\\
&\because a_0+a_1\cdot x+a_2\cdot x^2+a_3\cdot x^3+\ldots=0\\
&\Rightarrow a_i\ldots=0
\end{align}
$$