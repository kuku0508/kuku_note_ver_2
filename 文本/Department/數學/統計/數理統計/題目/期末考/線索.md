...
#### 問他的MLE是不是最小充分
Since MLE $\hat{\theta}$ of $\theta$ exists uniquely, and $T(\underline{X})$ is sufficient for $\theta$.
Hence, $\hat{\theta}$ is a function g of $T(\underline{X})$.

And 
$\because$ $\hat{\theta}$ is a one-to-one function of $T(\underline{X})$ $\frac{1}{\bar{X}+1}$ 
$\therefore$ 