...
$$
\begin{align}
f_{\underline{X}\mid T}(\underline{x}\mid t)=\frac{f_\underline{X}(x;\theta)}{f_T(t;\theta)}&=\frac{\prod^n_{i=1}\theta(1-\theta)^x}{\binom{n+t-1}{t}\theta^n(1-\theta)^t}\quad(Hence,T\sim NegBin(n,))\\
...
\end{align}
$$
$\because$ where h(x)=1 and $g(T(\underline{X}),\theta)=\theta^n(1-\theta)^T$  。
